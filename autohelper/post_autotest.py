...
gammaR_benchmark = '/home/zhuoyli/benchmarks/Mo/gamma/112_20l'
...
class PlotFig:
  ...
  def plot(self, lines: np.ndarray, pic_name='my_plot',
    ...
    ax.grid(True)
    plt.tight_layout()
    plt.savefig(f'{pic_name}.pdf', transparent=True, bbox_inches='tight', )
...
def run(props, strategy_list):
    for prop in props:
        ...
        elif prop in ['gamma', 'gammaA', 'gammaB', 'gammaR']:
    ...
    while True:
        ...
        if get_prop == 'all':
            props = ['relax', 'elastic', 'eos', 'cohesive', 'surface',
                     'vacancy', 'interstitial', 'gamma', 'gammaA', 'gammaB', 'gammaR']
            break
        elif get_prop == 'end':
            break
        else:
